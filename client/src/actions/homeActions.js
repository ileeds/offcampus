...
export function fetchHomes(fakeHomes) {
  return dispatch => {
    return fetch(`/homes`).then((response) => response.json()).then((data) => dispatch(homesReceived(data.data))).catch((e) => console.log(e));
  }
}
...
export function fetchHome(id) {
  return dispatch => {
    ...
    return fetch(`/homes/${id}`).then((response) => response.json()).then((data) => dispatch(homeReceived(data.data))).catch((e) => console.log(e));
  }
}
